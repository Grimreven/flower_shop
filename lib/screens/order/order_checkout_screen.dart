...
class _OrderCheckoutScreenState extends State<OrderCheckoutScreen> {
  ...
  Future<SbpBank?> _showSbpBankPicker() async {
    final SbpBank? selectedBank = await showModalBottomSheet<SbpBank>(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
      backgroundColor: Colors.transparent,
      builder: (BuildContext context) {
        return const SbpBankPickerSheet();
      },
    );
  ...
  Future<void> _confirmOrder() async {
    ...
    if (deliveryMethod == DeliveryMethod.delivery) {
      if (_localAddresses.isEmpty) {
        _showCheckoutWarning('У вас нет сохранённых адресов. Добавьте адрес доставки.');
        return;
      }
    ...
    if (selectedPayment.isSbp) {
      final SbpBank? selectedBank = await _showSbpBankPicker();
      ...
      paymentProvider = selectedBank.name;
    }
      ...
      child: Row(
        children: [
          ClipRRect(
            ...
            child: Container(
              color: isDark ? AppColors.darkSurfaceSoft : const Color(0xFFF8EFF3),
              child: Image.network(
  ...
  Widget _buildDeliveryMethodBlock(BuildContext context) {
    final bool isDark = Theme.of(context).brightness == Brightness.dark;
  ...
  Widget _buildAddressBlock(BuildContext context) {
    final bool isDark = Theme.of(context).brightness == Brightness.dark;
  ...
  Widget _buildPaymentBlock(BuildContext context) {
    return _block(
      context,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                SliderTheme(
                  ...
                  child: Slider(
                    ...
                    divisions:
                    summary.allowedBonuses > 0 ? summary.allowedBonuses : 1,
                    label: '$bonusToUse',
  ...
  Widget _buildTotalGradientCard(
      BuildContext context,
      CheckoutSummary summary,
      ) {
    final bool isDark = Theme.of(context).brightness == Brightness.dark;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              if (deliveryMethod == DeliveryMethod.delivery)
                _buildAddressBlock(context),
              _buildPaymentBlock(context),
              _block(
                context,
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
              ...
              _buildBonusBlock(context, summary),
              _buildTotalsBlock(context, summary),
              _buildTotalGradientCard(context, summary),
